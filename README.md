...
│   └── dynamic_detection.py # Dynamic content detection utilities
└── scraper/
...
2. **Install dependencies**:
   ...
   ```
3. **Install browser for patchright**:
   ```bash
   patchright install chromium
   ```
4. **Setup API key**: Create `api key.txt` with your Google AI API key:
   ```
...
*Note: Use `python main.py --list-sports` to see the current list*
